...
class Protein:
  ...
  dssp: np.ndarray = None # [num_res]; optional & computed with mdtraj
  
  local_flex: np.ndarray = None # [num_res, num_atom_type] optional for local flexibility conditioning

  std_local_flex: np.ndarray = None # [num_res, num_atom_type] optional for standard deviation of local flexibility conditioning
  ...
  def __post_init__(self):
    ...
    if self.b_factors is None:
      self.b_factors = np.zeros_like(self.atom_mask)


def from_pdb_string(pdb_str: str, chain_id: Optional[str] = None, calc_dssp:bool=False, 
                    loc_flex:bool=False, std_flex:bool=False) -> Protein:
  ...
  Modified: 
    included a dssp entry in the Protein object to store the secondary structure information
  """
