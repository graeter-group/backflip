...
def get_metrics(predictions, targets, config):
    '''
    Expects targets and predictions to be dictionaries with {target_type: list of numpy arrays with shape (*batch,num_res).}
    '''
    metrics = {}
    for out in config.node_outputs:
        profile_type = out['name']
        print(f"Computing metrics for node output: {profile_type}")
        if profile_type == "None":
            continue
        metrics[profile_type] = get_node_metrics(targets[profile_type], predictions[profile_type])
    
    for out in config.edge_outputs:
        profile_type = out['name']
        metrics[profile_type] = get_edge_metrics(targets[profile_type], predictions[profile_type])   
    return metrics

def get_node_metrics(target, prediction):
    '''
    ...
    if any(isinstance(t, torch.Tensor) for t in target):
        target = [t.cpu().numpy() for t in target]
    if any(isinstance(p, torch.Tensor) for p in prediction):
        prediction = [p.cpu().numpy() for p in prediction]
    ...
    epoch_metrics = {
        "global-r": pearson_R,
        "global-mae": mae,
        "per-target-mean-r": per_target_avg_r,
        "per-target-mean-mae": per_target_avg_mae,
    }

    return epoch_metrics

def get_edge_metrics(target, prediction):

    '''
    Expects target and prediction to be lists of 2d torch tensor/numpy arrays, each of which representing prediction and target for a single protein.
    '''
    assert isinstance(target, list) and isinstance(prediction, list), f"Expected target and prediction to be lists, got {type(target)} and {type(prediction)}"
    assert len(target) == len(prediction), f"Expected target and prediction to have the same length, got {len(target)} and {len(prediction)}"
    ...
    for target, prediction in zip(target, prediction):        
        per_target_corr.append(np.corrcoef(target.flatten(), prediction.flatten())[0,1])
        # print(target.shape, prediction.shape)
        per_target_mae.append(np.sqrt(mean_absolute_error(target.flatten(), prediction.flatten())))
        
    per_target_mae = np.mean(per_target_mae)
    per_target_corr = np.mean(per_target_corr)

    epoch_metrics = {
        "per-target-mean-corr": per_target_corr,
        "per-target-mean-mae": per_target_mae,
    }
    return epoch_metrics
