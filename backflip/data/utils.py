...
def read_npz(read_path: str, conf_idx: int = None, seed: int = 123):
    """
    Reads in the .npz file containing chain features
    Returns: chain_feats, npz
    - chain_feats: struc.AtomArray (biotite) object
    ...
    # chain_feats.b_factor = b
    return chain_feats, dict(npz)
...
def align_structures(
    ...
    # reference_positions -> Q [B / BN x 3]pwd
...
def parse_npz_feats(npz_feats:struc.AtomArray, 
    ...
    def _process_chain_id(npz_feats):
        chain_prot = process_chain_npz(npz_feats, dssp=calc_dssp, dataset_type=dataset_type)
        
        chain_dict = dataclasses.asdict(chain_prot)

        feat_names = CHAIN_FEATS + (['dssp'] if calc_dssp else CHAIN_FEATS)
        if calc_dssp:
            feat_names += ['dssp']

        feat_dict = {x: chain_dict[x] for x in feat_names}
...
def process_chain_npz(npz_feats: struc.AtomArray, 
                      dssp=None, 
                      dataset_type:str=None) -> Protein:
    '''
    npz_feats: struc.AtomArray extracted from a npz file from read_npz()
    dssp: None, cannot be computed for the feats extracted from .npz without writing creating a .pdb file
    REMARK: dataset_type is a temporary fix to handle non-atlas dataset; smarter would be to re-write ATLAS data processing...
    '''	
    if dataset_type != 'ATLAS':
        # have only N, CA, C only
        allowed_atoms = ['N', 'CA', 'C']
        npz_feats = npz_feats[np.isin(npz_feats.atom_name, allowed_atoms)]
...
def save_tite_as_pdb(protein_array, pdb_name:str, out_dir:str):
    '''
    Saves the selected chain as a clean .pdb file
    '''
    output = io.StringIO()
    file = pdb_io.PDBFile()
    file.set_structure(protein_array)
    file.write(output)
    string_pdb = output.getvalue()
    loc_pdb_save = os.path.join(out_dir, f'{pdb_name}_selected.pdb')
    with open(loc_pdb_save, 'w') as file:
        file.write(string_pdb)
    return None

def reshape_bfacs(b_facs, atom37_ox):
    '''
    Reshape b_facs [N*4] tensor to [N, 37] tensor
    '''
    test_bfacs = np.zeros(atom37_ox.shape[:-1])

    for i in range(atom37_ox.shape[0]):
        atoms_encoding = np.where(atom37_ox[i] != 0)[0]
        unique_atoms_encoding = np.array(list(set(atoms_encoding)))

        if unique_atoms_encoding.size > 0:
            test_bfacs[i, unique_atoms_encoding] = b_facs[0, i]  # Ensure we access a single value per index `i`
